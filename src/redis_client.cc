...
void RedisClient::set_value(const std::string& key, const std::string& value) const {
  reply_ = (redisReply*) redisCommand(context_, "SET %s %b", key.c_str(), value.c_str(), value.size());
  clean_reply();
}
...
void RedisClient::set_hashmap(const std::string& key, const Normalizers& hashmap) const {
  ...
  for (const auto& kv : hashmap) {
    ...
    reply_ = (redisReply*) redisCommand(context_, "HSET %s %s %b", key.c_str(), kv.first.c_str(), val_ptr, val_size);
    clean_reply();
  }
}

Normalizers RedisClient::get_hashmap(const std::string& key, int values_size) const {
  reply_ = (redisReply*) redisCommand(context_, "HKEYS %s", key.c_str());
  ...
  for (const auto& hkey : hkeys) {
    reply_ = (redisReply*) redisCommand(context_, "HGET %s %s", key.c_str(), hkey.c_str());
